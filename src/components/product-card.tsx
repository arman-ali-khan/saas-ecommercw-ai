...
export function AlsoBuyBlock({ product_ids, title, username }: { product_ids: string[], title?: string, username: string }) {
    const [products, setProducts] = useState<Product[]>([]);
    const [quantities, setQuantities] = useState<{ [key: string]: number }>({});
    const [isLoading, setIsLoading] = useState(true);
    const { toast } = useToast();
    ...
    return (
        <Card className="my-8">
            <CardHeader><CardTitle>{title || 'Also Buy'}</CardTitle></CardHeader>
