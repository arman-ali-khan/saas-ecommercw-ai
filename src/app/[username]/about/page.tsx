...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const { data: page } = await supabase.from('pages').select('title').eq('site_id', site.id).eq('slug', 'about').eq('is_published', true).single();
...
export default async function AboutPage({ params }: Props) {
  ...
  const { data: page } = await supabase.from('pages').select('title, content').eq('site_id', site.id).eq('slug', 'about').eq('is_published', true).single();
