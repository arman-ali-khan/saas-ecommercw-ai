...
import { Plus, Edit, Trash2, Loader2, X, AlertTriangle, ExternalLink, Palette, CheckCircle2 } from 'lucide-react';
...
export default function SaasThemesManagerPage() {
    ...
    const form = useForm<ThemeFormData>({
        ...
        defaultValues: { title: '', subtitle: '', preview_link: '', image_url: '', is_active: true, is_default: false },
    });
    ...
    useEffect(() => {
        if (isFormOpen) {
            if (selectedTheme) {
                ...
                form.reset({ title: '', subtitle: '', preview_link: '', image_url: '', is_active: true, is_default: false });
            }
        }
            ...
            {isFormOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-xl bg-background rounded-2xl shadow-2xl border flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-300">
                        ...
                        <div className="p-6 overflow-y-auto">
                            <Form {...form}>
                                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                                    ...
                                    <FormField control={form.control} name="subtitle" render={({ field }) => (<FormItem><FormLabel className="font-bold">Subtitle / Description</FormLabel><FormControl><Textarea placeholder="Briefly describe the look and feel." {...field} rows={3} /></FormControl></FormItem>)} />
                                    <FormField control={form.control} name="preview_link" render={({ field }) => (<FormItem><FormLabel className="font-bold">Preview URL</FormLabel><FormControl><Input placeholder="https://..." {...field} /></FormControl><FormMessage /></FormItem>)} />
                        ...
                        <div className="p-6 border-t flex justify-end gap-3 shrink-0 bg-muted/10">
                            ...
                            <Button onClick={form.handleSubmit(onSubmit)} disabled={isSubmitting} className="rounded-xl px-10 font-bold shadow-lg shadow-primary/20">
                                {isSubmitting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Saving...</> : "Save Theme"}
                            </Button>
                        </div>
                    </div>
                </div>
